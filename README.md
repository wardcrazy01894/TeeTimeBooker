...
3. It polls for available slots, picks the slot **closest to the midpoint** of the 08:45–10:00 ET window (midpoint-distance sort), and POSTs the booking (on a 4xx it tries the next-ranked slot). At a competitive drop Mangrove Bay also **blind-POSTs** the known morning grid at T0 — firing up to `scheduler.blind_post_max_count` (default 1, matching the shipped configs: burst-of-one, since ForeUP's 1-reservation/day rule rejects surplus POSTs anyway) book POSTs for the in-window slot(s) nearest the window midpoint, keeping the best and cancelling any extras — to beat the search round-trip; only if none book does a single fresh search run as the fallback (set `blind_post_max_count = 0` to disable)
