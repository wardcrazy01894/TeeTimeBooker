...
- **Snapshot trust — `ForeUpAdapter.snapshot_trusted` (`ReservationSnapshotHealth`, MULTIUSER_PLAN §7.5 / E6, MU-4).** `list_reservations()` reads the POST /login response cache; a soft-failed login, a non-JSON 200, or a JSON success whose `reservations` is missing / not a list all leave that cache empty or stale while looking healthy. The flag is True only when the latest login parsed a real list (an empty list counts), reset before every real login attempt. Read only by tenant code (never by the TOML engine); `list_reservations()` is unchanged (`tests/test_foreup_snapshot_trust.py`).
